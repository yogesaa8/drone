    ...
    return (
        <group ref={group} scale={config.size}>
            ...
            <mesh castShadow receiveShadow>
                ...
                <meshStandardMaterial color={config.body} metalness={0.6} roughness={0.4} />
            </mesh>

            {/* Top sensor */}
            <mesh position={[0, 0.28, 0]}>
                <cylinderGeometry args={[0.18, 0.22, 0.18, 16]} />
                <meshStandardMaterial color={config.accent} metalness={0.8} roughness={0.3} />
            </mesh>
            ...
            {armPositions.map((pos, i) => {
                const dirX = pos[0] > 0 ? 1 : -1;
                const dirZ = pos[2] > 0 ? 1 : -1;
                const len = Math.hypot(pos[0], pos[2]);
                const angle = Math.atan2(pos[2], pos[0]);
                return (
                    <group key={i}>
                        {/* arm */}
                        <mesh position={[pos[0] / 2, 0, pos[2] / 2]} rotation={[0, -angle, 0]}>
...
const DroneViewer = ({variant}) => {
    return (
        ...
        >
            ...
            {/* ground grid */}
            <gridHelper args={[20, 20, "#3a4a2a", "#1a1f15"]} position={[0, -0.6, 0]} />
